...
```{r illustration-plot, echo = FALSE, fig.align = 'default', fig.cap = '\\label{fig:comp_illustration}Illustration of the computational principles of the simulations. We plot the network activation when stimulated by a recurring unit {\\em ABC}. (a) On the first occurrence of the unit, no associations have been formed yet. Hence, when $A$ is presented, $A$ (but no other items) becomes active, and then decays, though some activation persists even while $C$ is presented. Likewise, $B$ and $C$ become active upon presentation, and then decay. The initial activation is weaker for $B$ and $C$ than for $A$ due to the presence of inhibitory interactions; this is because, for $A$, no other potentially inhibiting representations are active yet, while other activated items (e.g., $A$) have inhibitory input for $B$ and $C$. (b) On the last occurrence, associations between the items have been formed. When encountering external stimulation, the activation of $B$ and $C$ is thus great than that of $A$, because $B$ and $C$ (but not $A$) receive excitatory input from the strongly associated, preceeding items . (c) Weights at the end of the familiarization phase. The connection weights between adjacent items are stronger than those between non-adjacent items (i.e., between $A$ and $C$).'}
    ...
    plot_grid(first_word_first_occ_plot, 
                   first_word_last_occ_plot, 
                   weight_plot,
                   nrow=3,
                   labels = "auto") %>%
...
```{r r basic-experiment-global-create-plot_combined-fw-plot,}
...
## Supplementary Information 3: Experiments using the activation in the test-items instead of the global activation
...
### Experiments with a basic stream: Words, part-words, tested forwards and back-words
```{r basic-experiment-create_diff}

diff_basic <- cbind(
    l_act = data.frame (l_act = test_act_sum_basic$l_act),
    
    # Adjacent FW TP: Words vs. Part-Words (Forward)
    w_pw1_fw = make_diff_score(test_act_sum_basic,
                               "1-2-3", "2-3-4",
                               TRUE),
    w_pw2_fw = make_diff_score(test_act_sum_basic,
                               "1-2-3", "3-4-5",
                               TRUE),
    
    # Adjacent BW TP: Words vs. Part-Words (Backward)
    w_pw1_bw = make_diff_score(test_act_sum_basic,
                               "3-2-1", "4-3-2",
                               TRUE),
    w_pw2_bw = make_diff_score(test_act_sum_basic,
                               "3-2-1", "5-4-3",
                               TRUE),
    
    # Non-adjacent FW TP: Rule-Words vs. Class-Words (Forward)
    rw_cw_fw1 = make_diff_score(test_act_sum_basic,
                                "1-4-3", "1-4-9",
                                TRUE),
    rw_cw_fw2 = make_diff_score(test_act_sum_basic,
                                "1-19-3", "1-19-9",
                                TRUE),
    
    # Non-adjacent BW TP: Rule-Words vs. Class-Words (Backward)
    rw_cw_bw1 = make_diff_score(test_act_sum_basic,
                                "3-4-1", "9-4-1",
                                TRUE),
    rw_cw_bw2 = make_diff_score(test_act_sum_basic,
                                "3-19-1", "9-19-1",
                                TRUE)
) %>%
    as.data.frame()
...
#### Adjacent and non-adjacent forward TPs
In this section, we seek to demonstrate that the network is sensitive to basic forward TPs among and non-adjacent items. Again, to demonstrate a sensitivity to TPs among adjacent items, the network will be tested on units  and part-units. Likewise, the demonstration of a sensitivity to TPs among *non*-adjacent items is inspired by the paradigm by [@Endress-AXC] and will be tested on rule-units vs. class-units, either with a middle item that appear during familiarization or with a novel middle item. 

```{r basic-experiment-create-plot_diff-fw}
selected_cols_fw <- c(
    "w_pw1_fw", "w_pw2_fw",
    "rw_cw_fw1", "rw_cw_fw2")
...
diff_basic_fw_plot <- add_signif_to_plot(
    diff_basic_fw_plot,
    diff_basic,
    selected_cols_fw)
...
```{r basic-experiment-create-plot_diff-bw}
selected_cols_bw <- c(
    "w_pw1_bw", "w_pw2_bw",
    "rw_cw_bw1", "rw_cw_bw2")
...
diff_basic_bw_plot <- add_signif_to_plot(
    diff_basic_bw_plot,
    diff_basic,
    selected_cols_bw)
...
```{r basic-experiment-evaluate_diff-fw}

diff_basic_fw_p_values <- 
    lapply (L_ACT,
            function (CURRENT_L){
                diff_basic %>%
                    filter (l_act == CURRENT_L) %>% 
...
diff_basic_fw_p_values <- do.call ("rbind", 
                                   diff_basic_fw_p_values )

```

```{r basic-experiment-evaluate_diff-bw}

diff_basic_bw_p_values <- 
    lapply (L_ACT,
            function (CURRENT_L){
                diff_basic %>%
                    filter (l_act == CURRENT_L) %>% 
...
diff_basic_bw_p_values <- do.call ("rbind", 
                                   diff_basic_bw_p_values )

```

```{r basic-experiment-create-plot_p_sim-fw}

p_sim_basic_fw_plot <- diff_basic_fw_p_values %>%
    filter (Statistic == "p.simulations") %>%
    dplyr::select(-c("Statistic")) %>%
        melt (id.vars = "l_act",
          variable.name= "ItemType",
...
```{r basic-experiment-create-plot_p_sim-bw}

p_sim_basic_bw_plot <- diff_basic_bw_p_values %>%
    filter (Statistic == "p.simulations") %>%
    dplyr::select(-c("Statistic")) %>%
        melt (id.vars = "l_act",
          variable.name= "ItemType",
...
```{r basic-experiment-create-plot_diff-fw-plot, echo = FALSE, fig.align = 'default', fig.cap = write.caption.diff.scores ("basic", "basic_diff_fw", "forward", "activation in the test items")}
...
```{r basic-experiment-create-plot_p_sim-fw-plot, echo = FALSE, fig.align = 'default', fig.cap = write.caption.p.sim("basic", "basic_p_sim_fw", "forward", "activation in the test items")}
...
```{r basic-experiment-evaluate_diff-print, results='hide'}

diff_basic_fw_p_values %>%
    rbind (., diff_basic_bw_p_values) %>%
    rename_stuff_in_tables %>%
    mutate(Statistic = italisize_for_tex (Statistic)) %>%  
    docxtools::format_engr(sigdig=3) %>%
    knitr::kable(
...
```{r basic-experiment-sign-pattern-print, results='hide'}
get_sign_pattern_from_results(L_ACT, 
                              diff_basic_fw_p_values) %>%
    rbind (., get_sign_pattern_from_results(L_ACT, 
                              diff_basic_bw_p_values)) %>% 
    ...
    knitr::kable(
        ...
        booktabs = TRUE, 
        escape = FALSE,
        caption = '\\label{tab:basic_sign_pattern} Pattern of significance for the different forgetting rates and comparisons (Unit vs. Part-Unit: *ABC* vs. *BC:D* and *ABC* vs. *C:DE*; Rule-Unit vs. Class-Unit: *AGC* vs. *AGF* and *AXC* vs. *AXF*), for items presented in forward and backward order, and using the activation of the elements of the test-items as a measure of the network\'s familiarity with the items. +, - and 0 represent, respectively, a significant preference for the target item, a significant preference against the target item, or no significant preference, as evaluated by Wilcoxon tests on the relevant difference scores. Numbers indicate the proportion of simulations preferring target-items; bold-face numbers indicate significance in a binomial test.') %>%
    kableExtra::kable_styling(latex_options = c("striped", 
                                                "scale_down",
                                                "repeat_header")) %>%
    kableExtra::pack_rows("Forward", 
...
As shown in Figure \ref{fig:basic_diff_fw} and  \ref{fig:basic_p_sim_fw}, the results are very similar to those based on the global network activation reported above: The network fails for very low and very high forgetting parameters, and successeds on all comparisons with intermediate forgetting parameters. Numerically speaking, the results are similar to those used above as well. 

#### Adjacent and non-adjacent backward TPS
Again, we test the network's ability to track backward TPs by familiarizing the network with the same streams as in the previous section, but playing the test-items in reverse order (e.g., *CBA* instead of *ABC*).

```{r basic-experiment-create-plot_diff-bw-plot, echo = FALSE, fig.align = 'default', fig.cap = write.caption.diff.scores ("basic", "basic_diff_bw", "backward", "activation in the test items")}
...
```{r basic-experiment-create-plot_p_sim-bw-plot, echo = FALSE, fig.align = 'default', fig.cap = write.caption.p.sim("basic", "basic_p_sim_bw", "backward", "activation in the test items")}

p_sim_basic_bw_plot %>% print.plot

```


As shown in Figure \ref{fig:basic_diff_bw} and = \ref{fig:basic_p_sim_bw},  the results are very similar to those based on the global network activation reported above: The network fails for very low and very high forgetting parameters, and successeds on all comparisons with intermediate forgetting parameters. Numerically speaking, the results are similar to those used above as well. 

### Experiments with Phantom-Units
As mentioned above, the experiments presented so far confound TPs and frequency of occurrence: Units do not only have stronger TPs than part-units, but they also occur more frequently. Among the control experiments for this issue [@Aslin1998;@Endress-Phantoms;@Endress-Phantoms-Vision], our computational experiments are inspired by [@Endress-Phantoms] and [@Endress-Phantoms-Vision]. We thus expose the network to a six unit stream inspired by [@Endress-Phantoms] and [@Endress-Phantoms-Vision]. Following this, we test the network on units, phantom-units and part-units.

```{r phantoms-experiment-create_diff}

diff_phantoms <- cbind(
    l_act = data.frame (l_act = test_act_sum_phantoms$l_act),
    
    # Words vs. Part-Words 
    w_pw1 = make_diff_score(test_act_sum_phantoms,
                            "1-2-7", "2-7-8",
                            TRUE),
    w_pw2 = make_diff_score(test_act_sum_phantoms,
                            "1-2-7", "7-8-2",
                            TRUE),
    
    # Phantom-Words vs. Part-Words 
    phw_pw1 = make_diff_score(test_act_sum_phantoms,
                              "1-2-3", "2-7-8",
                              TRUE),
    phw_pw2 = make_diff_score(test_act_sum_phantoms,
                              "1-2-3", "7-8-2",
                              TRUE),
    
    # Words vs. Phantom-Words
    w_phw = make_diff_score(test_act_sum_phantoms,
                            "1-2-7", "1-2-3",
                            TRUE)
) %>%
    as.data.frame()
...
```{r phantom-experiment-create-plot_diff}
selected_cols_phantoms <- c("w_pw1", "w_pw2",
                            "phw_pw1", "phw_pw2",
                            "w_phw")
...
diff_phantoms_plot <- add_signif_to_plot(
    diff_phantoms_plot,
    diff_phantoms,
    selected_cols_phantoms)
...
```{r phantom-experiment-evaluate_diff}

diff_phantoms_p_values <- 
    lapply (L_ACT,
            function (CURRENT_L){
                diff_phantoms %>%
                    filter (l_act == CURRENT_L) %>% 
...
diff_phantoms_p_values <- do.call ("rbind", 
                                      diff_phantoms_p_values )


```

```{r phantom-experiment-create-plot_p_sim}

p_sim_phantoms_plot <- diff_phantoms_p_values %>%
    filter (Statistic == "p.simulations") %>%
    dplyr::select(-c("Statistic")) %>%
        melt (id.vars = "l_act",
          variable.name= "ItemType",
...
```{r phantom-experiment-create-plot_diff-plot, echo = FALSE, fig.align = 'default', fig.cap = write.caption.diff.scores ("phantoms", "phantoms_diff", "forward", "activation in the test items")}
...
```{r phantom-experiment-create-plot_p_sim-plot, echo = FALSE, fig.align = 'default', fig.cap = write.caption.p.sim("phantoms", "phantoms_p_sim", "forward", "activation in the test items")}
...
```{r phantom-experiment-evaluate_diff-print, results='hide'}
...
get_sign_pattern_from_results(L_ACT, 
    ...
    knitr::kable(
        ...
        booktabs = TRUE, 
        escape = FALSE,
        caption = '\\label{tab:phantoms_sign_pattern} Pattern of significance for the different forgetting rates and comparisons, and using the activation of the elements of the test-items as a measure of the network\'s familiarity with the items. +, - and 0 represent, respectively, a significant preference for the target item, a significant preference against the target item, or no significant preference, as evaluated by Wilcoxon tests on the relevant difference scores. Numbers indicate the proportion of simulations preferring target-items; bold-face numbers indicate significance in a binomial test.') %>%
    kableExtra::kable_styling(latex_options = c("striped", 
                                                "scale_down",
                                                "repeat_header")) 
...
As shown in Figure \ref{fig:phantoms_diff} and \ref{fig:phantoms_p_sim_fw}, the results are very similar to those based on the global network activation reported above: The network fails for very low and very high forgetting parameters, and prefers units and phantom-units over part-units roughly to the same extent for medium and high forgetting rates. As in [@Endress-Phantoms] and [@Endress-Phantoms-Vision], it thus more sensitive to differences in TPs than to differences in frequency of occurrence. In contrast, the network does not seem to discriminate between units and phantom-units, replicating [@Endress-Phantoms] and [@Endress-Phantoms-Vision] results. 
### Detailed results
Table \ref{tab:basic_diff} provides detailed results for the simulations in terms of descriptive statistics and statistical tests for the simulation testing the recognition of (forward and backward) units, part-units, rule-units and class-units.

Table \ref{tab:phantoms_diff} provides similar results for the simulations testing the recognition of units, part-units and phantom-units.

```{r basic-experiment-evaluate_diff-print2, ref.label='basic-experiment-evaluate_diff-print', echo=FALSE, results='markup'}

```

```{r phantom-experiment-evaluate_diff-print2, ref.label='phantom-experiment-evaluate_diff-print', echo=FALSE, results='markup'}

```


# Unused referencess
 [@Carey1978;@Markson1997] 
 
[@Medina2011;@Stevens2017;@Yu2007].

[@Beckman1986;@Brentari2011;@Endress-cross-seg;@Fenlon2008;@Nespor1986;@Pilon1981;@Selkirk1984;@Selkirk1986]

## Stuff about AGL

For example, when healthy participants are exposed to consonant strings  generated by a (finite-state) grammar, they can discriminate grammatical from ungrammatical strings even when the letter set is changed [@Reber1969] and even when analoguous strings are presented in a different modality [@Altmann1995]. Critically, amnesia patients can classify items as grammatical or ungrammatical despite severely impaired declarative memory for individual items [@Knowlton1992;@Knowlton1994]. 

If this had been true:

* memory deficit => no sl
* sl deficit => intact learning 

Role of loops/repetitions

* [@Gomez2000;@Tunney2001]


AGL is intact in Parkinson's disease

* https://www.sciencedirect.com/science/article/pii/S0028393202000271
* https://www.researchgate.net/publication/277172753_Preserved_artificial_grammar_learning_in_Parkinson's_disease
* https://www.sciencedirect.com/science/article/pii/S0028393202000271
* https://psycnet.apa.org/doiLanding?doi=10.1037%2F0735-7044.113.2.235

Critical difference might be feedback
* https://www.ncbi.nlm.nih.gov/pubmed/16359647




```{r save-stuff, echo = FALSE}
